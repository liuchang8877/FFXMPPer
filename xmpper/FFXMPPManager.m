...
- (BOOL)connect
{
    ...
    //获得用户名密码
    myJID       = [[NSUserDefaults alloc] objectForKey:USER_NAME];
    myPassword  = [[NSUserDefaults alloc] objectForKey:USER_PASS];;
    
    NSLog(@"connect---username %@, userpass:%@",myJID,myPassword);
...
- (void)setTheRegisteredInfo:(NSString *)myUserName pass:(NSString *)myUserPass {
    ...
    NSString *jid = [[NSString alloc] initWithFormat:@"%@@%@", myUserName, myUserPass];
    [xmppStream setMyJID:[XMPPJID jidWithString:jid]];
    NSError *error=nil;
    if (![xmppStream registerWithPassword:myUserPass error:&error])
    {
